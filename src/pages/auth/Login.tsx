...
import { loginWithEmail, loginWithGoogle, getFriendlyAuthErrorMessage } from '@/firebase/auth';
...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success('Welcome back to AeroTask');
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Check if user has completed onboarding (team selected)
      const existingProfile = await getUserProfile(uid);
      const needsOnboarding = !existingProfile?.team;

      success('Signed in with Google');
      navigate(needsOnboarding ? '/onboarding' : '/user/dashboard');
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
            Internal task management & performance platform
          </p>
        </div>
        ...
        {/* Security / Admin Note */}
        <div className="mt-6 p-3 rounded-xl bg-zinc-100/80 dark:bg-zinc-800/40 border border-zinc-200/60 dark:border-zinc-700/60 flex items-start gap-2.5 text-left text-xs text-zinc-600 dark:text-zinc-400">
          <ShieldCheck className="w-4 h-4 text-brand-600 dark:text-brand-400 shrink-0 mt-0.5" />
          <span>
            <strong>Role-Based Access:</strong> All accounts register as regular team members by default. Administrative access is granted securely by organization admins.
          </span>
        </div>
      </div>
    </div>
